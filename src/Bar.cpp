...
void
CBar::setTimeSig(int top, int bottom)
{
  ...
  m_beatLength =
    (CMidiFile::getPulsesPerQuarterNote() * 4) / m_currentTimeSigBottom;
  m_barLength = m_beatLength * getTimeSigTop();
}

int
CBar::addDeltaTime(int ticks)
{
...
CBar::goToBarNumer()
{
  ...
  ticks = static_cast<int>((m_playFromBar - getCurrentBarPos()) * m_beatLength *
                             SPEED_ADJUST_FACTOR +
                           1);
...
void
CBar::setPlayFromBar(double bar)
{
  m_playFromBar = bar;
...
void
CBar::setPlayFromBar(int bar, int beat, int ticks)
{
...
void
CBar::setPlayUptoBar(double endBar)
{
  setLoopingBars(endBar - m_playUptoBar);
}

void
CBar::setLoopingBars(double bars)
{
  if (bars < 0.0)
    bars = 0;
  m_loopingBars = bars;
