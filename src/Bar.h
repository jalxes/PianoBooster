...
#define EVENT_BITS_forceFullRedraw                                             \
  0x0002 // force the whole screen to be redrawn
...
#define EVENT_BITS_UptoBarReached                                              \
  0x0010 // Used for looping when playing between two bars.
...
public:
  ...
  // You MUST clear the time sig to 0 first before setting an new start Time
  // Signature at the start of the piece of music
  ...
  {
    return m_barCounter +
           static_cast<double>(m_beatCounter) / m_currentTimeSigBottom +
           static_cast<double>(m_deltaTime) /
             (m_beatLength * m_currentTimeSigBottom * SPEED_ADJUST_FACTOR);
  }
...
private:
  ...
  void setupEnableFlags()
  {
    ...
    m_enablePlayFromBar =
      (m_enableLooping || m_playFromBar > 0.0) ? true : false;
  }
