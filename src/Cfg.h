...
public:
  ...
  static float playZoneX()
  {
    return scrollStartX() + (staveEndX() - scrollStartX()) * 0.4f;
  }
  ...
  {
    return 8000;
  } // the time in msec before everything goes quiet
  static int chordNoteGap()
  {
    return 10;
  } // all notes in a cord must be spaced less than this a gap
  static int chordMaxLength()
  {
    return 20;
  ...
  {
    return CColor(0.5, 0.6, 1.0);
  } // purple 0.6, 0.6, 1.0
  static CColor playedBadColor()
  {
    return CColor(0.8, 0.3, 0.8);
  } // orange 0.7, 0.0, 0.0
  static CColor playedStoppedColor()
  {
    return CColor(1.0, 0.8, 0.0);
  } // bright orange
  ...
  static void setStaveEndX(float x) { m_staveEndX = x; }
