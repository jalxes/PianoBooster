...
int CChord::m_cfg_highestPianoNote =
  127; // The highest note on the users piano keyboard;
int CChord::m_cfg_lowestPianoNote = 0;

whichPart_t
CNote::findHand(int midiNote,
                int midiChannel,
                int whichChannel,
                whichPart_t whichPart)
{
  ...
  if (midiChannel != whichChannel) {
    ...
    if (CNote::hasPianoPart(whichChannel) == false ||
        CNote::hasPianoPart(midiChannel) == false)
      return PB_PART_none;
  }
...
void
CChord::addNote(whichPart_t part, int note, int duration)
{
...
void
CNote::setChannelHands(int left, int right)
{
...
bool
CChord::searchChord(int note, int transpose)
{
  int i;
...
int
CChord::trimOutOfRangeNotes(int transpose)
{
  int i;
  int removedNotes = 0;
  ...
  for (i = 0; i < MAX_CHORD_NOTES; i++) {
    ...
    if (!isNotePlayable(getNote(i).pitch(), transpose) ||
        !isHandPlayable(getNote(i).part()))
...
bool
CFindChord::findChord(CMidiEvent midi, int channel, whichPart_t part)
{
  ...
  if ((m_noteGapTime >= m_cfg_ChordNoteGap ||
       m_cordSpanGapTime > m_cfg_ChordMaxLength) &&
  ...
  if (midi.type() == MIDI_NOTE_ON) {
    ...
    if (hand != PB_PART_none) {
      ...
      m_currentChord.setDeltaTime(m_noteGapTime +
                                  m_currentChord.getDeltaTime());
