...
#define MAX_CHORD_NOTES                                                        \
  20 // The maximum notes in a chord well we only have 10 fingers
...
public:
  ...
  static int getHandChannel(whichPart_t whichPart)
  {
    return (whichPart == PB_PART_right) ? m_rightHandChannel
                                        : m_leftHandChannel;
  }
  static bool hasPianoPart(int chan)
  {
    return (m_leftHandChannel == chan || m_rightHandChannel == chan) ? true
                                                                     : false;
  }
...
{
public:
  int highestNote; // The highest note in the range (often on the users piano
                   // keyboard)
  int lowestNote;
};
...
public:
  ...
  {
    m_cfg_highestPianoNote =
      highestNote; // The highest note on the users piano keyboard;
...
private:
  ...
  int m_length;
  static int
    m_cfg_highestPianoNote; // The highest note on the users piano keyboard;
