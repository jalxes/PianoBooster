...
void
CConductor::midiEventInsert(CMidiEvent event)
{
...
int
CConductor::midiEventSpace()
{
  return m_songEventQueue->space();
}
void
CConductor::channelSoundOff(int channel)
{
...
void
CConductor::trackSoundOff(int trackNumber)
{
  channelSoundOff(track2Channel(trackNumber));
}

void
CConductor::allSoundOff()
{
  int channel;
...
void
CConductor::muteChannel(int channel, bool state)
{
  if (channel < 0 || channel >= MAX_MIDI_CHANNELS)
    return;
...
void
CConductor::mutePart(int part, bool state)
{
  int channel;
...
int
CConductor::calcBoostVolume(int channel, int volume)
{
  int returnVolume;
  bool activePart;
  ...
  if (CNote::hasPianoPart(m_activeChannel)) {
    ...
    {
      if (channel == CNote::leftHandChan() &&
          CNote::getActiveHand() != PB_PART_right)
        activePart = true;
      if (channel == CNote::rightHandChan() &&
          CNote::getActiveHand() != PB_PART_left)
        activePart = true;
...
CConductor::outputBoostVolume()
{
  ...
  for (chan = 0; chan < MAX_MIDI_CHANNELS; chan++) {
    ...
    midi.controlChangeEvent(
      0, chan, MIDI_MAIN_VOLUME, calcBoostVolume(chan, -1));
    playTrackEvent(midi);
  }
  outputPianoVolume();
}
...
void
CConductor::transpose(int transpose)
{
...
void
CConductor::activatePianistMutePart()
{
...
void
CConductor::mutePianistPart(bool state)
{
...
void
CConductor::setActiveHand(whichPart_t hand)
{
  ...
  for (i = 0; i < m_wantedChord.length(); i++) {
    ...
    m_scoreWin->setPlayedNoteColor(
      note, Cfg::playedStoppedColor(), m_chordDeltaTime);
  }
  findSplitPoint();
  forceScoreRedraw();
}
...
void
CConductor::setPlayMode(playMode_t mode)
{
  m_playMode = mode;
...
void
CConductor::setActiveChannel(int channel)
{
...
void
CConductor::testWrongNoteSound(bool enable)
{
...
void
CConductor::playMusic(bool start)
{
  m_playing = start;
  allSoundOff();
  if (start) {
    ...
    /*
    const unsigned char gsModeEnterData[] =  {0xf0, 0x41, 0x10, 0x42, 0x12,
...
// tacks will eventually allow for more than the 16 midi channels (eg with two
// mid devices)
void
CConductor::playTrackEvent(CMidiEvent event)
{
...
void
CConductor::playTransposeEvent(CMidiEvent event)
{
...
void
CConductor::resetWantedChord()
{
  m_wantedChord.clear();
  ppDEBUG_CONDUCTOR(
    ("resetWantedChord m_chordDeltaTime %d m_playingDeltaTime %d",
     m_chordDeltaTime,
     m_playingDeltaTime));
...
// switch modes if we are playing well enough (i.e. don't slow down if we are
// playing late)
void
CConductor::setFollowSkillAdvanced(bool enable)
{
  ...
  if (getLatencyFix() > 0) {
    ...
    m_cfg_earlyNotesPoint =
      m_cfg_imminentNotesOffPoint; // disable the early notes
  }
...
void
CConductor::turnOnKeyboardLights(bool on)
{
  int note;
  int i;
  CMidiEvent event;
...
bool
CConductor::validatePianistNote(const CMidiEvent& inputNote)
{
...
void
CConductor::playWantedChord(CChord chord, CMidiEvent inputNote)
{
  int pitch;
...
bool
CConductor::validatePianistChord()
{
 ...
 */
void
CConductor::expandPianistInput(CMidiEvent inputNote)
{
  if (m_playMode == PB_PLAY_MODE_rhythmTapping) {
    CChord chord;
    int i;
    ...
    if (inputNote.type() == MIDI_NOTE_ON || inputNote.type() == MIDI_NOTE_OFF) {
      ...
      whichPart_t targetPart =
        (inputNote.note() >= MIDDLE_C) ? PB_PART_right : PB_PART_left;
      for (i = 0; i < chord.length(); i++) {
...
void
CConductor::pianistInput(CMidiEvent inputNote)
{
  bool goodSound = true;
  ...
  whichPart_t hand;
  hand =
    (inputNote.note() >= m_pianistSplitPoint) ? PB_PART_right : PB_PART_left;
  ...
  if (inputNote.type() == MIDI_NOTE_ON) {
    ...
    if (validatePianistNote(inputNote) == true) {
      ...
      int pianistTiming;
      if ((cfg_timingMarkersFlag && m_followSkillAdvanced) ||
          m_playMode == PB_PLAY_MODE_rhythmTapping)
      ...
      if (validatePianistChord() == true) {
        ...
        if (m_rating.isAccuracyGood() || m_playMode == PB_PLAY_MODE_playAlong)
          setFollowSkillAdvanced(
            true); // change the skill level only when they are good enough
        else
      ...
      if (m_playing == true) {
        ...
        if (m_settings->followThroughErrors() &&
            m_playMode == PB_PLAY_MODE_followYou) // If the setting is checked,
                                                  // errors cause following too
        {
          if (m_chordDeltaTime <=
              -m_cfg_playZoneEarly) // We're hitting bad notes, but earlier than
                                    // the zone (so ignore them)
          {
            m_piano->clear();
            m_savedNoteQueue->clear();
            m_savedNoteOffQueue->clear();
          ...
          {
            // Register the wrong note in the ratings calculation (if not as
            // missed notes, I don't believe it's factored in)
            missedNotesColor(Cfg::pianoBadColor());
            m_rating.lateNotes(m_wantedChord.length() -
                               m_goodPlayedNotes.length());
            setEventBits(EVENT_BITS_forceRatingRedraw);
            fetchNextChord(); // Skip through the wrong note and continue to the
                              // next

            // Was the next note the one keyed by accident (dyslexia)?  If so,
            // validate it instead and continue as usual.
            if (m_wantedChord.searchChord(
                  inputNote.note(),
                  m_transpose)) // replaces validatePianistNote ignoring
                                // out-of-zone
            {
              m_goodPlayedNotes.addNote(hand, inputNote.note());
              m_piano->addPianistNote(hand, inputNote, true);
              ...
              if (validatePianistChord() == true) {
                ...
                m_rating.calculateAccuracy();
                m_settings->pianistActive();
                if (m_rating.isAccuracyGood() ||
                    m_playMode == PB_PLAY_MODE_playAlong)
                  setFollowSkillAdvanced(true); // change the skill level only
            ...
            // Clear & ignore any further slips until in range of the next
            // note's zone
            m_piano->clear();
            m_savedNoteQueue->clear();
            m_savedNoteOffQueue->clear();
  ...
  if (goodSound == true || m_cfg_wrongNoteSound < 0) {
    if (m_cfg_rightNoteSound >=
        0) // don't play anything if the sound is set to -1 (none)
    {
      bool playDrumBeat = false;
      if (inputNote.channel() != MIDI_DRUM_CHANNEL) {
        if (cfg_rhythmTapping != PB_RHYTHM_TAP_drumsOnly ||
            m_playMode != PB_PLAY_MODE_rhythmTapping) {
      ...
      if (cfg_rhythmTapping != PB_RHYTHM_TAP_mellodyOnly &&
          m_playMode == PB_PLAY_MODE_rhythmTapping)
        playDrumBeat = true;
      ...
      if (playDrumBeat) {
        ...
        inputNote.setNote((hand == PB_PART_right)
                            ? m_cfg_rhythmTapRightHandDrumSound
                            : m_cfg_rhythmTapLeftHandDrumSound);
        playTrackEvent(inputNote);
      }
    }
  } else {
  ...
  int pianoSound = (goodSound == true) ? m_cfg_rightNoteSound :
  m_cfg_wrongNoteSound;
...
void
CConductor::addDeltaTime(int ticks)
{
...
void
CConductor::outputSavedNotesOff()
{
  while (m_savedNoteOffQueue->length() > 0)
    playTransposeEvent(
      m_savedNoteOffQueue->pop()); // Output the saved note off events
}

// untangle the sound in case there is any notes off just after we have stopped
void
CConductor::findImminentNotesOff()
{
  int i;
  CMidiEvent event;
  int aheadDelta = 0;
  ...
  while (deltaAdjust(m_playingDeltaTime) + aheadDelta >
         m_cfg_imminentNotesOffPoint) {
...
void
CConductor::missedNotesColor(CColor color)
{
  int i;
  CNote note;
  for (i = 0; i < m_wantedChord.length(); i++) {
    ...
    if (m_goodPlayedNotes.searchChord(note.pitch(), m_transpose) == false)
      m_scoreWin->setPlayedNoteColor(
        note.pitch() + m_transpose, color, m_chordDeltaTime);
  }
}

void
CConductor::realTimeEngine(int mSecTicks)
{
  int type;
  ...
  if (m_realTimeEventBits | EVENT_BITS_newBarNumber) {
    ppDEBUG_CONDUCTOR(("m_savedNoteQueue %d m_playingDeltaTime %d",
                       m_savedNoteQueue->space(),
                       m_playingDeltaTime));
    ppDEBUG_CONDUCTOR(("getfollowState() %d  %d %d",
                       getfollowState(),
                       m_leadLagAdjust,
                       m_songEventQueue->length()));
  }
#endif
  ...
  while (m_playingDeltaTime >= m_leadLagAdjust) {
    ...
    if (type == MIDI_PB_tempo) {
      ...
      ppLogDebug("Midi Time Signature %d/%d",
                 m_nextMidiEvent.data1(),
                 m_nextMidiEvent.data2());
      ...
      if (!hasPianistKeyboardChannel(channel)) {
        if (getfollowState() >= PB_FOLLOW_earlyNotes &&
            (m_playMode == PB_PLAY_MODE_followYou ||
             m_playMode == PB_PLAY_MODE_rhythmTapping) &&
            !seekingBarNumber() && m_followSkillAdvanced == false) {
          ...
          ppDEBUG_CONDUCTOR(("playEvent() chan %d type %d note %d",
                             m_nextMidiEvent.channel(),
                             m_nextMidiEvent.type(),
                             m_nextMidiEvent.note(),
                             m_songEventQueue->length()));
        }
      }
    }
...
CConductor::rewind()
{
  ...
  m_cfg_stopPointBeginner =
    CMidiFile::ppqnAdjust(-0); // was -3; // stop just after the beat
  m_cfg_stopPointAdvanced =
    CMidiFile::ppqnAdjust(-15); // was -3; // stop just after the beat
  m_cfg_imminentNotesOffPoint =
    CMidiFile::ppqnAdjust(-15); // look ahead and find an Notes off coming up

  // Annie song 25

  m_cfg_playZoneEarly = CMidiFile::ppqnAdjust(Cfg::playZoneEarly()) *
                        SPEED_ADJUST_FACTOR; // when playing along
  m_cfg_playZoneLate =
    CMidiFile::ppqnAdjust(Cfg::playZoneLate()) * SPEED_ADJUST_FACTOR;
}

void
CConductor::init2(CScore* scoreWin, CSettings* settings)
{
  int channel;
