...
@brief          The realtime midi engine. Send notes to the midi device and
responds to the midi keyboard.
...
public:
  ...
  bool hasPianistKeyboardChannel(int chan)
  {
    return (m_pianistGoodChan == chan || m_pianistBadChan == chan) ? true
                                                                   : false;
  }
  ...
  {
    m_realTimeEventBits |= bits;
  } // don't change the other bits
...
protected:
  ...
  eventBits_t m_realTimeEventBits; // used to signal real time events to the
                                   // caller of task()
...
private:
  ...
  int m_silenceTimeOut; // used to create silence if the student stops for toooo
                        // long
  ...
  int m_boostVolume;
  int m_pianoVolume;
  int m_activeChannel; // The current part that is being displayed (used for
                       // boost and activatePianistMutePart)
  int m_savedMainVolume[MAX_MIDI_CHANNELS];
  static playMode_t m_playMode;
  int m_skill;
  bool m_mutePianistPart;
  int m_latencyFix; // Try to fix the latency (put the time in msec, 0 disables
                    // it)
