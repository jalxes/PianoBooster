...
void
CDraw::oneLine(float x1, float y1, float x2, float y2)
{
  glBegin(GL_LINES);
  glVertex2f((x1), (y1));
  glVertex2f((x2), (y2));
...
void
CDraw::drawStaveExtentsion(CSymbol symbol,
                           float x,
                           int noteWidth,
                           bool playable)
{
...
#define scaleGlVertex(xa, xb, ya, yb)                                          \
  glVertex2f(((xa)*1.2) + (xb), ((ya)*1.2) + (yb))

#ifndef NO_USE_FTGL
void
CDraw::renderText(float x, float y, const char* s)
{
...
void
CDraw::drawNoteName(int midiNote, float x, float y, int type)
{
  ...
  const QString n[7] = { tr("C"), tr("D"), tr("E"), tr("F"),
                         tr("G"), tr("A"), tr("B") };
...
void
CDraw::drawStaveNoteName(CSymbol symbol, float x, float y)
{
...
void
CDraw::checkAccidental(CSymbol symbol, float x, float y)
{
  int accidental;
  const int xGap = 16;
  ...
  if (symbol.getAccidentalModifer() == PB_ACCIDENTAL_MODIFER_force) {
    // Force the display of an accidental including naturals if it is the same
    // bar
...
void
CDraw::drawSymbol(CSymbol symbol, float x, float y, CSlot* slot)
{
  ...
  switch (symbol.getType()) {
    ...
    case PB_SYMBOL_noteHead:
      ...
      glBegin(GL_LINE_STRIP);
          glVertex2f(x,  y);
          glVertex2f(x + CMidiFile::ppqnAdjust(symbol.getMidiDuration()) *
      HORIZONTAL_SPACING_FACTOR, y); glEnd(); drColor(color);
      */
    ...
    case PB_SYMBOL_barLine:
      x += BEAT_MARKER_OFFSET *
           HORIZONTAL_SPACING_FACTOR; // the beat markers where entered early so
                                      // now move them correctly
      glLineWidth(4.0);
      drColor((m_displayHand == PB_PART_left) ? Cfg::staveColorDim()
                                              : Cfg::staveColor());
      oneLine(x,
              CStavePos(PB_PART_right, 4).getPosYRelative(),
              x,
              CStavePos(PB_PART_right, -4).getPosYRelative());
      drColor((m_displayHand == PB_PART_right) ? Cfg::staveColorDim()
                                               : Cfg::staveColor());
      oneLine(x,
              CStavePos(PB_PART_left, 4).getPosYRelative(),
              x,
              CStavePos(PB_PART_left, -4).getPosYRelative());
      break;
    ...
    case PB_SYMBOL_barMarker:
      ...
      oneLine(x,
              CStavePos(PB_PART_right, m_beatMarkerHeight).getPosYRelative(),
              x,
              CStavePos(PB_PART_left, -m_beatMarkerHeight).getPosYRelative());
...
void
CDraw::drawSymbol(CSymbol symbol, float x)
{
  drawSymbol(symbol, x, symbol.getStavePos().getPosY());
}

void
CDraw::drawSlot(CSlot* slot)
{
  CStavePos stavePos;
  // int av8Left = slot->getAv8Left();
  for (int i = 0; i < slot->length(); i++) {
    stavePos.notePos(slot->getSymbol(i).getHand(),
                     slot->getSymbol(i).getNote());
    // ppLogTrace ("compileSlot len %d id %2d next %2d time %2d type %2d note
    // %2d", slot->length(), slot->m_displayListId, slot->m_nextDisplayListId,
    // slot->getDeltaTime(), slot->getSymbol(i).getType(),
    // slot->getSymbol(i).getNote());

    drawSymbol(
      slot->getSymbol(i),
      0.0,
      stavePos.getPosYRelative()); // we add this  back when drawing this symbol
  }
}

void
CDraw::drawStaves(float startX, float endX)
{
  int i;
  ...
  drColor((m_displayHand != PB_PART_left) ? Cfg::staveColor()
                                          : Cfg::staveColorDim());
  glBegin(GL_LINES);
  ...
  drColor((m_displayHand != PB_PART_right) ? Cfg::staveColor()
                                           : Cfg::staveColorDim());
  for (i = -4; i <= 4; i += 2) {
...
void
CDraw::drawKeySignature(int key)
{
  ...
  while (key != 0) {
    if (key > 0) {
      if (i < arraySize(sharpLookUpRight)) {
        drColor((m_displayHand != PB_PART_left) ? Cfg::noteColor()
                                                : Cfg::noteColorDim());
        pos = CStavePos(PB_PART_right, sharpLookUpRight[i]);
        drawSymbol(CSymbol(PB_SYMBOL_sharp, pos),
                   Cfg::keySignatureX() + gapX * i);
      }
      if (i < arraySize(sharpLookUpLeft)) {
        drColor((m_displayHand != PB_PART_right) ? Cfg::noteColor()
                                                 : Cfg::noteColorDim());
      ...
      if (i < arraySize(flatLookUpRight)) {
        ...
        drawSymbol(CSymbol(PB_SYMBOL_flat, pos),
                   Cfg::keySignatureX() + gapX * i);
      }
