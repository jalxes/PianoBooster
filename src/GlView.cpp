...
// Try to make sure this runs a bit faster than the screen refresh rate of 60z
// (or 16.6 msec)
#define SCREEN_FRAME_RATE 12 // That 12 msec or 83.3 frames per second

#define REDRAW_COUNT                                                           \
  ((m_cfg_openGlOptimise >= 2)                                                 \
     ? 1                                                                       \
     : 2) // there are two gl buffers but redrawing once is best (set 2 with
          // buggy gl drivers)
...
QSize
CGLView::minimumSizeHint() const
{
  return QSize(200, 200);
}

QSize
CGLView::sizeHint() const
{
  return QSize(200, 800); // fixme this does not work
}

void
CGLView::stopTimerEvent()
{
  m_allowedTimerEvent = false;
}

void
CGLView::startTimerEvent()
{
  m_allowedTimerEvent = true;
}
...
CGLView::drawTimeSignature()
{
  ...
  CDraw::drColor((CDraw::getDisplayHand() != PB_PART_left)
                   ? Cfg::noteColor()
                   : Cfg::noteColorDim());
  ...
  CDraw::drColor((CDraw::getDisplayHand() != PB_PART_right)
                   ? Cfg::noteColor()
                   : Cfg::noteColorDim());
...
CGLView::drawAccurracyBar()
{
  if (m_song->getPlayMode() == PB_PLAY_MODE_listen ||
      !m_settings->getWarningMessage().isEmpty())
    return;
...
CGLView::drawDisplayText()
{
  ...
  renderText(
    30, y + 6, 0, tr("Song:") + " " + m_song->getSongTitle(), m_timeRatingFont);
  /*
  char buffer[100];
...
void
CGLView::resizeGL(int width, int height)
{
  const int maxSoreWidth = 1024;
  const int staveEndGap = 20;
  const int heightAboveStave =
    static_cast<int>(CStavePos::verticalNoteSpacing() * MAX_STAVE_INDEX);
  const int heightBelowStave =
    static_cast<int>(CStavePos::verticalNoteSpacing() * -MIN_STAVE_INDEX);
  const int minTitleHeight = 20;
  const int minStaveGap = 120;
  int staveGap;
  int maxSoreHeight;

  // int space = height - (heightAboveStave + heightBelowStave + minTitleHeight
  // + minStaveGap); m_titleHeight = qBound(minTitleHeight, minTitleHeight +
  // space/2, 70);
  // staveGap = qBound(minStaveGap, minStaveGap+ space/2,
  // static_cast<int>(CStavePos::staveHeight() * 3));
  ...
  maxSoreHeight =
    heightAboveStave + heightBelowStave + staveGap + m_titleHeight;
...
void
CGLView::mousePressEvent(QMouseEvent* event)
{}
...
CGLView::initializeGL()
{
  ...
  if (!Cfg::quickStart) {
    renderText(10,
               10,
               QString("~"),
               m_timeRatingFont); // fixme this is a work around for a QT bug.
    renderText(10,
               10,
               QString("~"),
               m_timeSigFont); // this is a work around for a QT bug.
  }
...
void
CGLView::timerEvent(QTimerEvent* event)
{
  if (!m_allowedTimerEvent)
    return;
  ...
  if (m_eventBits != 0) {
    ...
    if ((m_eventBits & EVENT_BITS_newBarNumber) != 0)
      m_forcefullRedraw = m_forceRatingRedraw = m_forceBarRedraw =
        REDRAW_COUNT; // fixme this did not work so redraw everything
...
void
CGLView::mediaTimerEvent(int ticks)
{}
