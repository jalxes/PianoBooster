...
void
GuiKeyboardSetupDialog::init(CSong* song, CSettings* settings)
{
  m_song = song;
  m_settings = settings;
...
void
GuiKeyboardSetupDialog::updateInfoText()
{
  QString str;
  ...
  keyboardInfoText->append(
    "<span style=\"color:black\">" +
    tr("Choose the right and wrong sound for your playing.") + "</span>");
  if (!lowestNoteEdit->isEnabled())
    str = "<span style=\"color:black\">" +
          tr("You can use the PC keyboard instead of a MIDI keyboard; 'x' is "
             "middle C.") +
          "</span>";
  else if (noteRange > 0)
    str = "<span style=\"color:black\">" +
          QString(tr("Your keyboard range is <b>octaves %1</b> and "
                     "<b>semitones %2</b>; 60 is middle C."))
            .arg(noteRange / MIDI_OCTAVE)
            .arg(noteRange % MIDI_OCTAVE) +
          "</span>";
  else
    str = "<span style=\"color:red\">" +
          tr("Oops, you have <b>0 notes</b> on your keyboard!") + "</span>";

  keyboardInfoText->append(str);
}

void
GuiKeyboardSetupDialog::keyPressEvent(QKeyEvent* event)
{
...
void
GuiKeyboardSetupDialog::keyReleaseEvent(QKeyEvent* event)
{
  if (event->isAutoRepeat() == true)
    return;
