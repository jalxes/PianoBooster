...
void
GuiMidiSetupDialog::init(CSong* song, CSettings* settings)
{
  m_song = song;
  m_settings = settings;
  ...
  connect(audioDriverCombo,
          SIGNAL(currentIndexChanged(int)),
          this,
          SLOT(on_audioDriverCombo_currentIndexChanged(int)));

  enableFluidSynth->setChecked(
    m_settings->value("FluidSynth/enableFluidSynth", "false").toBool());
  connect(enableFluidSynth,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(on_enableFluidSynth_stateChanged(int)));

  if (m_settings->getFluidSoundFontNames().size() != 0) {
    masterGainSpin->setValue(
      m_settings->value("FluidSynth/masterGainSpin", "0.2").toDouble());
    bufferSizeSpin->setValue(
      m_settings->value("FluidSynth/bufferSizeSpin", "").toInt());
    bufferCountsSpin->setValue(
      m_settings->value("FluidSynth/bufferCountsSpin", "").toInt());
    reverbCheck->setChecked(
      m_settings->value("FluidSynth/reverbCheck", "false").toBool());
    chorusCheck->setChecked(
      m_settings->value("FluidSynth/chorusCheck", "false").toBool());

    audioDeviceLineEdit->setText(
      m_settings->value("FluidSynth/audioDeviceLineEdit", "").toString());
    for (int i = 0; i < audioDriverCombo->count(); i++) {
      if (audioDriverCombo->itemText(i) ==
          m_settings->value("FluidSynth/audioDriverCombo", "").toString()) {
        audioDriverCombo->setCurrentIndex(i);
        break;
      }
    }
    sampleRateCombo->setCurrentText(
      m_settings->value("FluidSynth/sampleRateCombo").toString());
  }
  ...
  connect(enableTimidity,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(on_enableTimidity_stateChanged(int)));

  // disabled checkbox timidityAlsa
  timidityAlsa->setEnabled(false);

  enableTimidity->setChecked(
    m_settings->value("TiMidity/enableTimidity", "false").toBool());
  timidityAlsa->setChecked(
    m_settings->value("TiMidity/timidityAlsa", "true").toBool());
  timidityLibaoMode->setChecked(
    m_settings->value("TiMidity/timidityLibaoMode", "true").toBool());
  timidityPcmDevice->setChecked(
    m_settings->value("TiMidity/timidityPcmDevice", "false").toBool());

  on_enableTimidity_stateChanged(enableTimidity->isChecked());
}

void
GuiMidiSetupDialog::updateMidiInfoText()
{

  midiInfoText->clear();

  if (midiInputCombo->currentIndex() == 0)
    midiInfoText->append("<span style=\"color:black\">" +
                         tr("If you don't have a MIDI keyboard you can use the "
                            "PC keyboard; 'X' is middle C.") +
                         "</span>");
  else if (midiInputCombo->currentText().contains("Midi Through",
                                                  Qt::CaseInsensitive))
    midiInfoText->append("<span style=\"color:#FF6600\">" +
                         tr("The use of Midi Through is not recommended!") +
                         "</span>");
  else
    midiInfoText->append("<span style=\"color:gray\">" +
                         tr("Midi Input Device:") + " " +
                         midiInputCombo->currentText() + "</span>");

  if (midiOutputCombo->currentText() == tr("None"))
    midiInfoText->append(
      "<span style=\"color:red\">" +
      tr("No Sound Output Device selected; Choose a Midi Output Device") +
      "</span>");
  else if (midiOutputCombo->currentText().contains("Midi Through",
                                                   Qt::CaseInsensitive))
    midiInfoText->append("<span style=\"color:#FF6600\">" +
                         tr("The use of Midi Through is not recommended!") +
                         "</span>");
  else if (midiOutputCombo->currentText().contains("Microsoft GS Wavetable",
                                                   Qt::CaseInsensitive))
    midiInfoText->append("<span style=\"color:#FF6600\">" +
                         tr("Note: the Microsoft GS Wavetable Synth introduces "
                            "an unwanted delay!.") +
                         "\n" + tr("(Try a latency fix of 150msc)") +
                         "</span>");
  else
    midiInfoText->append("<span style=\"color:gray\">" +
                         tr("Midi Output Device:") + " " +
                         midiOutputCombo->currentText() + "</span>");
...
void
GuiMidiSetupDialog::on_midiInputCombo_activated(int index)
{
  m_midiChanged = true;
  updateMidiInfoText();
}
...
void
GuiMidiSetupDialog::on_midiOutputCombo_activated(int index)
{
  m_midiChanged = true;
  updateMidiInfoText();
}
...
void
GuiMidiSetupDialog::on_latencyFixButton_clicked(bool checked)
{
  bool ok;
  int latencyFix = QInputDialog::getInt(
    this,
    tr("Enter a value for the latency fix in milliseconds"),
    tr("The latency fix works by running the music ahead of what you<br>"
       "are playing to counteract the delay within the sound generator.<br><br>"
       "You will need a piano <b>with speakers</b> that are <b>turned "
       "up</b>.<br><br>"
...
void
GuiMidiSetupDialog::accept()
{
  if (m_midiChanged) {

    m_settings->setValue("Midi/Input", midiInputCombo->currentText());
    m_song->openMidiPort(CMidiDevice::MIDI_INPUT,
                         midiInputCombo->currentText());
  ...
  if (m_latencyChanged) {
    if (m_latencyFix > 0) {
      int rightSound =
        m_settings->value("Keyboard/RightSound", Cfg::defaultRightPatch())
          .toInt();
      m_settings->setValue("Keyboard/RightSoundPrevious",
                           rightSound); // Save the current right sound
      // Mute the Piano if we are using the latency fix;
      m_settings->setValue("Keyboard/RightSound", 0);
      m_song->setPianoSoundPatches(
        -1, -2); // -1 means no sound and -2 means ignore this parameter
    } else {
      int previousRightSound =
        m_settings
          ->value("Keyboard/RightSoundPrevious", Cfg::defaultRightPatch())
          .toInt();
      m_settings->setValue("Keyboard/RightSound", previousRightSound);
      m_song->setPianoSoundPatches(previousRightSound,
                                   -2); // -2 means ignore this parameter
    }
    m_latencyChanged = false;
  }
  ...
  if (m_settings->getFluidSoundFontNames().size() == 0) {
    ...
    m_settings->setValue("FluidSynth/masterGainSpin",
                         QString::number(masterGainSpin->value(), 'f', 2));
    m_settings->setValue("FluidSynth/bufferSizeSpin", bufferSizeSpin->value());
    m_settings->setValue("FluidSynth/bufferCountsSpin",
                         bufferCountsSpin->value());
    ...
    m_settings->setValue("FluidSynth/audioDriverCombo",
                         audioDriverCombo->currentText());
    if (audioDriverCombo->currentText() == "alsa") {
      m_settings->setValue("FluidSynth/audioDeviceLineEdit",
                           audioDeviceLineEdit->text());
    } else {
      m_settings->setValue("FluidSynth/audioDeviceLineEdit", "");
    }
    m_settings->setValue("FluidSynth/sampleRateCombo",
                         sampleRateCombo->currentText());
  }
  m_settings->setValue("FluidSynth/enableFluidSynth",
                       enableFluidSynth->isChecked());
  ...
  if (!enableTimidity->isChecked()) {
    ...
    m_settings->setValue("TiMidity/timidityLibaoMode",
                         timidityLibaoMode->isChecked());
    m_settings->setValue("TiMidity/timidityPcmDevice",
                         timidityPcmDevice->isChecked());
  }
...
void
GuiMidiSetupDialog::updateFluidInfoText()
{
...
void
GuiMidiSetupDialog::setDefaultFluidSynth()
{
...
void
GuiMidiSetupDialog::on_fluidAddButton_clicked(bool checked)
{
  ...
  QString soundFontName =
    QFileDialog::getOpenFileName(this,
                                 tr("Open SoundFont2 File for fluidsynth"),
                                 lastSoundFont,
                                 tr("SoundFont2 Files (*.sf2)"));
  ...
  for (int i = 0; i < m_settings->getFluidSoundFontNames().size(); i++) {
    m_settings->setValue("FluidSynth/SoundFont2_" + QString::number(1 + i),
                         m_settings->getFluidSoundFontNames().at(i));
  }
}

void
GuiMidiSetupDialog::on_fluidRemoveButton_clicked(bool checked)
{
  if (soundFontList->currentRow() == -1)
    return;

  QStringList soundFontNames = m_settings->getFluidSoundFontNames();

  m_settings->removeFluidSoundFontName(
    soundFontNames.at(soundFontList->currentRow()));
...
void
GuiMidiSetupDialog::on_audioDriverCombo_currentIndexChanged(int index)
{
  if (audioDriverCombo->currentText() == "alsa") {
...
void
GuiMidiSetupDialog::on_enableFluidSynth_stateChanged(int status)
{
  if (!enableFluidSynth->isChecked()) {
    ...
    if (m_settings->getFluidSoundFontNames().size() > 0)
      fluidSettingsGroupBox->setEnabled(true);
    groupBox_3->setEnabled(true);
  }
}

void
GuiMidiSetupDialog::on_enableTimidity_stateChanged(int status)
{
  if (enableTimidity->isChecked()) {
