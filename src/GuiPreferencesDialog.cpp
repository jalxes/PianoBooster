...
void
GuiPreferencesDialog::initLanguageCombo()
{
#ifndef NO_LANGS
  ...
  for (int i = 0; i < listLang.size(); ++i) {
    ...
    if (rx.indexIn(fileInfo.fileName()) != -1) {
      ...
      if (rootLangs.value(lang_code)
            .toObject()
            .value("nativeName")
            .isString()) {
        languageName =
          rootLangs.value(lang_code).toObject().value("nativeName").toString();
      }
      ...
      if (m_settings->value("General/lang", QLocale::system().bcp47Name())
            .toString() == lang_code) {
...
void
GuiPreferencesDialog::init(CSong* song, CSettings* settings, CGLView* glView)
{
  m_song = song;
  m_settings = settings;
  m_glView = glView;
  ...
  int index =
    videoOptimiseCombo->count() - (m_glView->m_cfg_openGlOptimise + 1);
  ...
  courtesyAccidentalsCheck->setChecked(
    m_settings->displayCourtesyAccidentals());
  showTutorPagesCheck->setChecked(m_settings->isTutorPagesEnabled());
  followThroughErrorsCheck->setChecked(
    m_settings->isFollowThroughErrorsEnabled());
...
void
GuiPreferencesDialog::accept()
{
  m_glView->m_cfg_openGlOptimise =
    videoOptimiseCombo->count() - (videoOptimiseCombo->currentIndex() + 1);
  ...
  m_settings->setFollowThroughErrorsEnabled(
    followThroughErrorsCheck->isChecked());
  m_song->cfg_stopPointMode =
    static_cast<stopPointMode_t>(followStopPointCombo->currentIndex());
