...
void
GuiSidePanel::init(CSong* songObj, CTrackList* trackList, GuiTopBar* topBar)
{
  m_song = songObj;
  ...
  playMode_t skill =
    m_settings->value("SidePanel/skill", PB_PLAY_MODE_followYou).toInt();
  switch (skill) {
  ...
  on_rhythmTappingCombo_activated(
    m_settings->value("SidePanel/rhythmTapping", 0).toInt());
  rhythmTappingCombo->setCurrentIndex(m_song->cfg_rhythmTapping);

  repeatSong->setChecked(
    m_settings->value("SidePanel/repeatSong", false).toBool());
  connect(repeatSong,
          SIGNAL(stateChanged(int)),
          this,
          SLOT(on_repeatSong_released()));
...
void
GuiSidePanel::on_bookCombo_activated(int index)
{
  QString currentSong;

  m_settings->setCurrentBookName(bookCombo->currentText(),
                                 (index >= 0) ? true : false);
...
void
GuiSidePanel::on_songCombo_activated(int index)
{
  m_settings->setCurrentSongName(songCombo->currentText());
}

void
GuiSidePanel::on_rightHandRadio_toggled(bool checked)
{
  if (checked)
...
void
GuiSidePanel::on_bothHandsRadio_toggled(bool checked)
{
  if (checked)
...
void
GuiSidePanel::on_leftHandRadio_toggled(bool checked)
{
  if (checked)
...
void
GuiSidePanel::on_repeatSong_released()
{
  m_settings->setValue("SidePanel/repeatSong", repeatSong->isChecked());
}

void
GuiSidePanel::autoSetMuteYourPart()
{
  bool checked = false;
...
void
GuiSidePanel::setSongName(QString songName)
{
...
void
GuiSidePanel::setBookName(QString bookName)
{
...
void
GuiSidePanel::setCurrentHand(QString hand)
{
  if (hand == "left")
...
GuiSidePanel::updateTranslate()
{
  ...
  QVector<QWidget*> listCheckWidget{ label2,         listenRadio,
                                     rhythmTapRadio, followYouRadio,
                                     playAlongRadio, rightHandRadio,
                                     bothHandsRadio, leftHandRadio };
  ...
  for (QWidget* w : listCheckWidget) {
    ...
    if (lb)
      delta = fm.width(lb->text()) - lb->width();

    QRadioButton* rb = dynamic_cast<QRadioButton*>(w);
    if (rb)
      delta = fm.width(rb->text()) - (rb->width() - 32);

    if (delta > maxDeltaWidth)
      maxDeltaWidth = delta;
  }
...
void
GuiSidePanel::on_rhythmTappingCombo_activated(int index)
{
