...
void
CMidiDeviceFluidSynth::init()
{}

QStringList
CMidiDeviceFluidSynth::getMidiPortList(midiType_t type)
{

  if (type != MIDI_OUTPUT) // Only has an output
    return QStringList();

  // debugSettings(("getSongList %s + %d", qPrintable(getCurrentBookName()),
...
bool
CMidiDeviceFluidSynth::openMidiPort(midiType_t type, QString portName)
{
  closeMidiPort(MIDI_OUTPUT, -1);
  ...
  if (getMidiPortList(type).size() == 0)
    return false;
  ...
  // m_soundFontId = fluid_synth_sfload(m_synth, "VintageDreamsWaves-v2.sf2",
  // 0);
  ...
  for (int channel = 0; channel < MAX_MIDI_CHANNELS; channel++) {
    // fluid_synth_program_select(m_synth, channel, m_soundFontId, 0,
    // GM_PIANO_PATCH);
...
void
CMidiDeviceFluidSynth::closeMidiPort(midiType_t type, int index)
{
  if (type != MIDI_OUTPUT)
    return;
...
void
CMidiDeviceFluidSynth::playMidiEvent(const CMidiEvent& event)
{
  ...
  switch (event.type()) {
    ...
    case MIDI_CONTROL_CHANGE: // CONTROL_CHANGE:
      ...
      // ppLogTrace("MIDI_CONTROL_CHANGE %d %d %d", channel, event.data1(),
      // event.data2());
      break;
    ...
    case MIDI_PITCH_BEND: // PITCH_BEND:
      ...
      fluid_synth_pitch_bend(
        m_synth, channel, (event.data2() << 7) | event.data1());
      break;
...
int
CMidiDeviceFluidSynth::checkMidiInput()
{
  return 0;
}
...
CMidiEvent
CMidiDeviceFluidSynth::readMidiInput()
{
...
int
CMidiDeviceFluidSynth::midiSettingsSetStr(QString name, QString str)
{
  if (!m_fluidSettings)
    return 0;

  return fluid_settings_setstr(
    m_fluidSettings, (char*)qPrintable(name), (char*)qPrintable(str));
}

int
CMidiDeviceFluidSynth::midiSettingsSetNum(QString name, double val)
{
  if (!m_fluidSettings)
    return 0;
...
int
CMidiDeviceFluidSynth::midiSettingsSetInt(QString name, int val)
{
  if (!m_fluidSettings)
    return 0;
...
QString
CMidiDeviceFluidSynth::midiSettingsGetStr(QString name)
{
  char buffer[200];
  if (!m_fluidSettings)
    return QString();
...
double
CMidiDeviceFluidSynth::midiSettingsGetNum(QString name)
{
  if (!m_fluidSettings)
    return 0.0;
  double val;
...
int
CMidiDeviceFluidSynth::midiSettingsGetInt(QString name)
{
  if (!m_fluidSettings)
    return 0;
  int val = 0;
