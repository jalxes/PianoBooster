...
QString
CMidiDeviceRt::addIndexToString(QString name, int index)
{
  QString ret;
  QString idx;
  idx.setNum(index);
...
bool
CMidiDeviceRt::openMidiPort(midiType_t type, QString portName)
{
  unsigned int nPorts;
  QString name;
  RtMidi* midiDevice;
...
void
CMidiDeviceRt::closeMidiPort(midiType_t type, int index)
{
  if (type == MIDI_INPUT)
    m_midiin->closePort();
  else
...
void
CMidiDeviceRt::playMidiEvent(const CMidiEvent& event)
{
  if (m_midiPorts[1] < 0)
    return;
...
CMidiDeviceRt::readMidiInput()
{
  ...
  switch (m_inputMessage[0] & 0xf0) {
    case MIDI_NOTE_ON:
      if (m_inputMessage[2] != 0)
        ...
        midiEvent.noteOffEvent(
          0, channel, m_inputMessage[1], m_inputMessage[2]);
      break;
    ...
    case MIDI_NOTE_PRESSURE: // MIDI_CMD_NOTE_PRESSURE: //POLY_AFTERTOUCH:
      ...
      // midi_input_bytes[midi_input_length++] = channel |
      // MIDI_CMD_NOTE_PRESSURE; midi_input_bytes[midi_input_length++] =
      // ev->data.note.note; midi_input_bytes[midi_input_length++] =
      // ev->data.note.velocity;
      break;

    case MIDI_CONTROL_CHANGE: // CONTROL_CHANGE:
      midiEvent.controlChangeEvent(
        0, channel, m_inputMessage[1], m_inputMessage[2]);
      break;

    case MIDI_PROGRAM_CHANGE: // PROGRAM_CHANGE:
      // midiEvent.programChangeEvent(0, ev->data.control.channel,
      // ev->data.control.value);
      break;

    case MIDI_CHANNEL_PRESSURE: // AFTERTOUCH:
      // fixme fill in the blanks
      // midi_input_bytes[midi_input_length++] = ev->data.control.channel |
      // MIDI_CMD_CHANNEL_PRESSURE; midi_input_bytes[midi_input_length++] =
      // ev->data.control.value;
      break;
...
int
CMidiDeviceRt::midiSettingsSetStr(QString name, QString str)
{
  return 0;
}

int
CMidiDeviceRt::midiSettingsSetNum(QString name, double val)
{
  return 0;
}

int
CMidiDeviceRt::midiSettingsSetInt(QString name, int val)
{
  return 0;
}

QString
CMidiDeviceRt::midiSettingsGetStr(QString name)
{
  return QString();
}

double
CMidiDeviceRt::midiSettingsGetNum(QString name)
{
  return 0.0;
}

int
CMidiDeviceRt::midiSettingsGetInt(QString namel)
{
  return 0;
}
