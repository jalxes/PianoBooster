...
void
CMidiFile::openMidiFile(string filename)
{
  if (m_file.is_open())
    m_file.close();
  ...
  if (m_file.fail() == true) {
    QMessageBox::warning(
      0,
      QMessageBox::tr("Midi File Error"),
  ...
  if (getMidiError() != SMF_NO_ERROR)
    QMessageBox::warning(0,
                         QMessageBox::tr("Midi File Error"),
                         QMessageBox::tr("Midi file \"%1\" is corrupted")
...
bool
CMidiFile::checkMidiEventFromStream(int streamIdx)
{
...
CMidiEvent
CMidiFile::fetchMidiEventFromStream(int streamIdx)
{
