  ...
  , m_trackNumber(no)
{
  ...
  m_trackLength = m_trackLengthCounter +
                  8; // 4 bytes for the "MTrk" + 4 bytes for the track length
  m_trackEventQueue = new CQueue<CMidiEvent>(
    m_trackLength / 3); // The minimum bytes per event is 3
}

void
CMidiTrack::ppDebugTrack(int level, const char* msg, ...)
{
  va_list ap;
...
dword_t
CMidiTrack::readDataEvent(int expectedLength)
{
  int length;
...
void
CMidiTrack::ignoreSysexEvent(byte_t data)
{
  word_t length;
...
/* Time Signature */
void
CMidiTrack::readTimeSignatureEvent()
{
  byte_t len;
  ...
  event.metaEvent(readDelaTime(),
                  MIDI_PB_timeSignature,
                  timeSigNumerator,
                  1 << timeSigDenominator);
  m_trackEventQueue->push(event);
  __dt(ppDebugTrack(4,
                    "Key Signature %d/%d metronome %d quarter %d",
                    timeSigNumerator,
                    1 << timeSigDenominator,
                    b3,
                    b4));
...
/* Key Signature */
void
CMidiTrack::readKeySignatureEvent()
{
  byte_t len;
  CMidiEvent event;
  int keySig;
  ...
  keySig = static_cast<char>(
    readByte());         // force sign conversion The key sig 0=middle C
...
void
CMidiTrack::readMetaEvent(byte_t type)
{
  string text;
  __dt(dword_t data);
...
void
CMidiTrack::decodeSystemMessage(byte_t status, byte_t data1)
{
  switch (status) {
...
void
CMidiTrack::noteOffEvent(CMidiEvent& event,
                         int deltaTime,
                         int channel,
                         int pitch,
                         int velocity)
{
  ...
  if (noteOnEventPtr) {
    ...
    // ppLogDebug ("NOTE OFF chan %d pitch %d  currentTime %d Duration %d",
    // channel + 1, pitch, m_currentTime, duration);
  } else {
    ppLogWarn(
      "Missing note off duration Chan %d Note off %d", channel + 1, pitch);
  }
  ...
  switch (status & 0xf0) {
    ...
    case MIDI_NOTE_ON: /* Note on */
      ...
      if (data2 != 0) {
        ...
        event.setDuration(
          m_currentTime); // Set the duration to the current time for now
        // ppLogDebug ("NOTE ON  pitch %d m_currentTime %d event->getDuration()
        // %d", data1, m_currentTime, event.getDuration());
    ...
    case MIDI_NOTE_PRESSURE: /* Key pressure After touch (POLY_AFTERTOUCH)  3
                                bytes */
      data2 = readByte();
    ...
    case MIDI_PROGRAM_CHANGE: /* program change */
      ...
      m_trackEventQueue->push(event);
      __dt(
        ppDebugTrack(2, "Chan %d Program change %d", channel + 1, data1 + 1));
      break;
    ...
    case MIDI_CONTROL_CHANGE: /* Control Change */
      ...
      m_trackEventQueue->push(event);
      __dt(ppDebugTrack(
        2, "Chan %d Control Change %d %d", channel + 1, data1, data2));
      break;
