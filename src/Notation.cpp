...
bool
CSlot::addSymbol(CSymbol symbol)
{
  int i;
...
CNotation::nextBeatMarker()
{
  ...
  if (m_beatPerBarCounter == -1) // Sneak in a bar line
    slot.setSymbol(m_bar.getBeatLength() - cfg_barGap,
                   CSymbol(PB_SYMBOL_barLine, PB_PART_both, 0));
  else if (m_beatPerBarCounter == 0)
    slot.setSymbol(cfg_barGap, CSymbol(PB_SYMBOL_barMarker, PB_PART_both, 0));
  else
    slot.setSymbol(m_bar.getBeatLength(),
                   CSymbol(PB_SYMBOL_beatMarker, PB_PART_both, 0));
  return slot;
}

int
CNotation::nextMergeSlot()
{
  size_t i;
  CSlot nearestSlot;
  size_t nearestIndex;
...
accidentalModifer_t
CNotation::detectSuppressedNatural(int note)
{
  ...
  if (direction != 0) {
    ...
    m_noteState[note + direction].setBackLink(
      pNoteState); // point back to this note
  ...
  if (pBackLink) {
    ...
    pBackLink->setBarChange(
      -1); // this prevents further suppression on the original note
  }
...
void
CNotation::setupNotationParamaters()
{
  cfg_param[NOTATE_semiquaverBoundary] =
    CMidiFile::ppqnAdjust(DEFAULT_PPQN / 4 + 10);
  cfg_param[NOTATE_quaverBoundary] =
    CMidiFile::ppqnAdjust(DEFAULT_PPQN / 2 + 10);
  cfg_param[NOTATE_crotchetBoundary] = CMidiFile::ppqnAdjust(DEFAULT_PPQN + 10);
  cfg_param[NOTATE_minimBoundary] =
    CMidiFile::ppqnAdjust(DEFAULT_PPQN * 2 + 10);
  cfg_param[NOTATE_semibreveBoundary] =
    CMidiFile::ppqnAdjust(DEFAULT_PPQN * 4 + 10);
}

void
CNotation::calculateScoreNoteLength()
{
  ...
  for (int i = 0; i < slot->length(); i++) {
    ...
    // ie assume that this note ends at the exact time the following note
    // starts.
...
CNotation::nextSlot()
{
  ...
  if (m_mergeSlots[MERGESLOT_BEATMARK_INDEX].length() == 0) {
    ...
    // This inserts the beat marksers into the queue early (so they get drawn
    // underneath)
    m_mergeSlots[MERGESLOT_BEATMARK_INDEX].addDeltaTime(
      -CMidiFile::getPulsesPerQuarterNote() * BEAT_MARKER_OFFSET /
      DEFAULT_PPQN);
  }
...
void
CNotation::midiEventInsert(CMidiEvent event)
{
  if (m_findScrollerChord.findChord(event, m_displayChannel, PB_PART_both) ==
      true) {
...
CNotation::reset()
{
  ...
  m_earlyBarChangeDelta =
    cfg_earlBarLead; // We want to detect the bar change early
