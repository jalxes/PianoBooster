...
void
CPiano::spaceNoteBunch(unsigned int bottomIndex, unsigned int topIndex)
{
  ...
  if (range % 2 == 0) // test for an even number of notes
  {
    ...
    if (gap < minNameGap) {
      ...
      m_noteNameList[midPoint].posY +=
        gap; // then move the middle two notes apart by the same amout
...
void
CPiano::drawPianoInputNoteNames()
{
  unsigned int i;
  ...
  for (i = 0; i < m_noteNameListLength; i++) {
    drawNoteName(m_noteNameList[i].pitch,
                 Cfg::playZoneX() - PIANO_LINE_LENGTH_SHORT - 14,
                 m_noteNameList[i].posY,
                 m_noteNameList[i].type);
  }
}

void
CPiano::drawPianoInputLines(CChord* chord, CColor color, int lineLength)
{
  int i;
  ...
  for (i = 0; i < chord->length(); i++) {
    if (!m_rhythmTapping) {
      ...
      glLineWidth(3.0);
      oneLine(
        Cfg::playZoneX(), top.getPosY(), Cfg::playZoneX(), bottom.getPosY());
    }
  }
}

void
CPiano::spaceNoteNames()
{
...
void
CPiano::addNoteNameItem(float posY, int pitch, int type)
{
...
void
CPiano::addPianistNote(whichPart_t part, CMidiEvent midiNote, bool good)
{
  CStavePos stavePos;
  float posY;
  ...
  if (stavePos.getStaveIndex() >= MAX_STAVE_INDEX ||
      stavePos.getStaveIndex() <= MIN_STAVE_INDEX)
    return;
...
void
CPiano::removeNoteNameItem(int pitch)
{
  unsigned int i;
  ...
  for (i = 0; i < m_noteNameListLength; i++) {
    if (foundMatch == true)
      m_noteNameList[i - 1] =
        m_noteNameList[i]; // found a match so move every thing else up
...
bool
CPiano::removePianistNote(int note)
{
...
int
CPiano::pianistAllNotesDown()
{
  return m_goodChord.length() + m_badChord.length();
}

int
CPiano::pianistBadNotesDown()
{
...
void
CPiano::drawPianoInput()
{
  bool showNoteName = m_settings->showNoteNames();
  int lineLength =
    (showNoteName) ? PIANO_LINE_LENGTH_SHORT : PIANO_LINE_LENGTH_LONG;
...
void
CPiano::addSavedChord(CMidiEvent midiNote, CChord chord)
{
...
CChord
CPiano::removeSavedChord(int key)
{
  unsigned int i;
