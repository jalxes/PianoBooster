...
static int
set_realtime_priority(int policy, int prio)
{
...
QtWindow::QtWindow()
{
  ...
  m_song->setPianoSoundPatches(
    m_settings->value("Keyboard/RightSound", Cfg::defaultRightPatch()).toInt() -
      1,
    m_settings->value("Keyboard/WrongSound", Cfg::defaultWrongPatch()).toInt() -
      1,
    true);
  ...
  if (midiInputName.startsWith(tr("None")))
    ...
    CChord::setPianoRange(
      m_settings->value("Keyboard/LowestNote", 0).toInt(),
  ...
  if (m_settings->value("Display/OpenGlOptimise").toString() ==
      "true") // this used to be true for backward compatability
    m_settings->setValue("Display/OpenGlOptimise",
                         m_glWidget->m_cfg_openGlOptimise);
  if (m_settings->value("Display/OpenGlOptimise").toString() ==
      "false") // this used to be boolean for backward compatability
  {
  ...
  m_glWidget->m_cfg_openGlOptimise =
    m_settings
      ->value("Display/OpenGlOptimise", m_glWidget->m_cfg_openGlOptimise)
      .toInt();
  m_song->cfg_timingMarkersFlag =
    m_settings->value("Score/TimingMarkers", m_song->cfg_timingMarkersFlag)
      .toBool();
  m_song->cfg_stopPointMode = static_cast<stopPointMode_t>(
    m_settings->value("Score/StopPointMode", m_song->cfg_stopPointMode)
      .toInt());
  m_song->cfg_rhythmTapping = static_cast<rhythmTapping_t>(
    m_settings->value("Score/RtyhemTappingMode", m_song->cfg_rhythmTapping)
      .toInt());

  m_song->openMidiPort(CMidiDevice::MIDI_INPUT, midiInputName);
  m_song->openMidiPort(CMidiDevice::MIDI_OUTPUT,
                       m_settings->value("Midi/Output").toString());
...
QtWindow::displayUsage()
{
  ...
  fprintf(
    stdout,
    "      --Xtick-rate=RATE   Adjust the tick rate in mSec (experimental).\n");
...
int
QtWindow::decodeIntegerParam(QString arg, int defaultParam)
{
...
bool
QtWindow::validateIntegerParam(QString arg)
{
  int n = arg.lastIndexOf('=');
  if (n == -1 || (n + 1) >= arg.size())
    return false;
...
bool
QtWindow::validateIntegerParamWithMessage(QString arg)
{
  bool ok = validateIntegerParam(arg);
  if (!ok) {
    fprintf(stderr,
            "ERROR: Invalid parameter to a command line argument \"%s\".\n",
            qPrintable(arg));
    exit(0);
  }
  return ok;
...
void
QtWindow::decodeMidiFileArg(QString arg)
{
  ...
  if (!fileInfo.exists()) {
    QMessageBox::warning(
      0,
      tr("PianoBooster Midi File Error"),
    ...
    if (vaildMidiFile == true)
      ...
      QMessageBox::warning(0,
                           tr("PianoBooster Midi File Error"),
                           tr("\"%1\" is not a valid Midi file")
                             .arg(QString(fileInfo.absoluteFilePath())));
      exit(1);
    }
  }
}
...
QtWindow::decodeCommandLine()
{
  ...
  for (int i = 0; i < argList.size(); ++i) {
    ...
    if (arg.startsWith("-")) {
      ...
      else if (arg.startsWith("-h") || arg.startsWith("-?") ||
               arg.startsWith("--help")) {
        displayUsage();
        exit(0);
...
void
QtWindow::addShortcutAction(const QString& key, const char* method)
{
...
QtWindow::createActions()
{
  ...
  connect(
    m_setupKeyboardAct, SIGNAL(triggered()), this, SLOT(showKeyboardSetup()));
  ...
  connect(m_fullScreenStateAct,
          SIGNAL(triggered()),
          this,
          SLOT(onFullScreenStateAct()));
  ...
  connect(
    m_sidePanelStateAct, SIGNAL(triggered()), this, SLOT(toggleSidePanel()));
  ...
  connect(m_viewPianoKeyboard,
          SIGNAL(triggered()),
          this,
          SLOT(onViewPianoKeyboard()));
  ...
  connect(
    m_songDetailsAct, SIGNAL(triggered()), this, SLOT(showSongDetailsDialog()));
  ...
  for (int i = 0; i < MAX_RECENT_FILES; ++i) {
    ...
    connect(
      m_recentFileActs[i], SIGNAL(triggered()), this, SLOT(openRecentFile()));
  }
}

void
QtWindow::createMenus()
{
...
void
QtWindow::updateRecentFileActions()
{
...
QString
QtWindow::strippedName(const QString& fullFileName)
{
...
void
QtWindow::setCurrentFile(const QString& fileName)
{
...
QtWindow::website()
{
  QDesktopServices::openUrl(
    QUrl("https://github.com/captnfab/PianoBooster/tree/master/doc"));
}
...
QtWindow::help()
{
  ...
  msgBox.setText(tr(
    ...
    "<p>You need a <b>MIDI Piano Keyboard </b> and a <b>MIDI interface</b> for "
    "the PC. If you "
    "don't have a MIDI keyboard you can still try out PianoBooster using the "
    "PC keyboard, 'X' is "
    "middle C.</p>"

    "<p>To hear the music you will need a <b>General Midi sound "
    "synthesizer</b>. "
    "The \"Microsoft GS Wavetable software synthesizer\" that comes with "
    "Windows can be used "
    "but it introduces an unacceptable delay (latency). In Linux you can use "
    "<a href=\"http://www.fluidsynth.org\">FluidSynth</a> or <a "
    "href=\"http://timidity.sourceforge.net/\">Timidity</a></p>"

    "<p>PianoBooster works best with MIDI files that have separate left and "
    "right piano parts "
    "using MIDI channels 3 and 4."

    "<h3>Setting Up</h3>"

    "<p>First use the <i>Setup/Midi Setup</i> menu and in the dialog box "
    "select the MIDI input and MIDI "
    "output interfaces that match your hardware. "

    "Next use <i>File/Open</i> to open the MIDI file \".mid\" or a karaoke "
    "\".kar\" file. "
    "Now select whether you want to just <i>listen</i> to the music or "
    "<i>play along</i> on the piano keyboard by setting the <i>skill</i> level "
    "on the side panel. Finally when "
    "you are ready click the <i>play icon</i> (or press the <i>space bar</i>) "
    "to roll the music."

    "<h3>Hints on Playing the Piano</h3>"
    "<p>For hints on how to play the piano see: "
    "<a "
    "href=\"https://github.com/captnfab/PianoBooster/blob/master/doc/"
    "pianohints.md\" ><b>Piano Hints</b></a></p>"

    "<h3>More Information</h3>"
    "<p>For more help please visit the PianoBooster "
    "<a href=\"https://github.com/captnfab/PianoBooster/tree/master/doc\" "
    "><b>website</b></a>, "
    "the PianoBooster <a "
    "href=\"https://github.com/captnfab/PianoBooster/blob/master/doc/faq.md\" "
    "><b>FAQ</b></a> "
    "and the <a "
    "href=\"http://piano-booster.2625608.n2.nabble.com/"
    "Piano-Booster-Users-f1591936.html\" ><b>user forum</b></a>."
...
QtWindow::about()
{
  ...
  msgBox.setText(
    ...
    "<a href=\"https://github.com/captnfab/PianoBooster\" "
    "><b>https://github.com/captnfab/PianoBooster</b></a><br><br>" +
...
QString
QtWindow::displayShortCut(QString key, QString description)
{
  ...
  QString str = QString("<tr>"
                        ...
                        "</tr>")
                  .arg(description)
                  .arg(tr(m_settings->value(key).toString().toUtf8().data()));
  return str;
}

void
QtWindow::keyboardShortcuts()
{
  QMessageBox msgBox(this);
  msgBox.setWindowTitle(tr("PC Keyboard ShortCuts"));
  QString msg =
    tr("<h2><center>Keyboard shortcuts</center></h2>"
  ...
  msg +=
    displayShortCut("ShortCuts/PlayFromStart", tr("Play from start toggle"));
  ...
  msg += displayShortCut("ShortCuts/PreviousSong",
                         tr("Change to the Previous Song"));
  msg += displayShortCut("ShortCuts/NextBook", tr("Change to the Next Book"));
  msg += displayShortCut("ShortCuts/PreviousBook",
                         tr("Change to the Previous Book"));

  msg += tr("<tr><td>Fake Piano keys</td><td>X is middle C</td></tr>"
            "</table> </center><br>");
  msgBox.setText(msg);
...
QtWindow::open()
{
  ...
  QString fileName = QFileDialog::getOpenFileName(
    ...
    tr("Open Midi File"),
    dir,
    tr("Midi Files") + " (*.mid *.MID *.midi *.MIDI *.kar *.KAR)");
  if (!fileName.isEmpty()) {
...
void
QtWindow::closeEvent(QCloseEvent* event)
{
  if (m_song->playingMusic()) {
...
void
QtWindow::keyPressEvent(QKeyEvent* event)
{
...
void
QtWindow::keyReleaseEvent(QKeyEvent* event)
{
  if (event->isAutoRepeat() == true)
    return;
...
void
QtWindow::loadTutorHtml(const QString& name)
{
  if (name.isEmpty()) {
    m_tutorWindow->hide();
    m_tutorWindow->clear();
...
QtWindow::refreshTranslate()
{
...
#endif
#if defined(Q_OS_LINUX) || defined(Q_OS_UNIX)
  QApplication::applicationDirPath() + "/../share/games/" + QSTR_APPNAME +
    "/translations/";
#endif
#ifdef Q_OS_DARWIN
  ...
  if (!fileTestLocale.exists()) {
    localeDirectory =
      QString(PREFIX) + "/" + QString(DATA_DIR) + "/translations/";
  }

  QString locale =
    m_settings->value("General/lang", QLocale::system().bcp47Name()).toString();
  ...
  if (!translator.load(QSTR_APPNAME + QString("_") + locale, localeDirectory))
    if (!translator.load(QSTR_APPNAME + QString("_") + locale,
                         QApplication::applicationDirPath() + "/translations/"))
      translator.load(QSTR_APPNAME + QString("_") + locale,
                      QApplication::applicationDirPath());
  qApp->installTranslator(&translator);

  // set translator for music
  if (!translatorMusic.load(QString("music_") + locale, localeDirectory))
    if (!translatorMusic.load(QString("music_") + locale,
                              QApplication::applicationDirPath() +
                                "/translations/"))
      translatorMusic.load(QString("music_") + locale,
                           QApplication::applicationDirPath());
  ...
  for (auto& w : l2) {
    if (!w->toolTip().isEmpty())
      w->setToolTip(
        tr(listWidgetsRetranslateUi[w]["toolTip"].toStdString().c_str()));
    if (!w->whatsThis().isEmpty())
      w->setWhatsThis(
        tr(listWidgetsRetranslateUi[w]["whatsThis"].toStdString().c_str()));
    if (!w->windowTitle().isEmpty())
      w->setWindowTitle(
        tr(listWidgetsRetranslateUi[w]["windowTitle"].toStdString().c_str()));
    if (!w->statusTip().isEmpty())
      w->setStatusTip(
        tr(listWidgetsRetranslateUi[w]["statusTip"].toStdString().c_str()));
  }
  ...
  for (auto& w : l) {
    if (!w->toolTip().isEmpty())
      w->setToolTip(
        tr(listActionsRetranslateUi[w]["toolTip"].toStdString().c_str()));
    if (!w->whatsThis().isEmpty())
      w->setWhatsThis(
        tr(listActionsRetranslateUi[w]["whatsThis"].toStdString().c_str()));
    if (!w->statusTip().isEmpty())
      w->setStatusTip(
        tr(listActionsRetranslateUi[w]["statusTip"].toStdString().c_str()));
    if (!w->text().isEmpty())
      w->setText(tr(listActionsRetranslateUi[w]["text"].toStdString().c_str()));
  }
