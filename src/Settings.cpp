  ...
  , m_mainWindow(mainWindow)
{
  ...
  CNotation::setCourtesyAccidentals(
    value("Score/CourtesyAccidentals", false).toBool());
  m_followThroughErrorsEnabled =
    value("Score/FollowThroughErrors", false).toBool();

  // load Fluid settings

  QString soundFontNames_1 = value("FluidSynth/SoundFont2_1", "").toString();
  if (!soundFontNames_1.isEmpty())
    addFluidSoundFontName(soundFontNames_1);

  QString soundFontNames_2 = value("FluidSynth/SoundFont2_2", "").toString();
  if (!soundFontNames_2.isEmpty())
    addFluidSoundFontName(soundFontNames_2);
}

void
CSettings::setDefaultValue(const QString& key, const QVariant& value)
{
...
void
CSettings::init(CSong* song, GuiSidePanel* sidePanel, GuiTopBar* topBar)
{
  m_song = song;
...
void
CSettings::setNoteNamesEnabled(bool value)
{
...
void
CSettings::setColoredNotes(bool value)
{
...
void
CSettings::setTutorPagesEnabled(bool value)
{
...
void
CSettings::setCourtesyAccidentals(bool value)
{
...
void
CSettings::setFollowThroughErrorsEnabled(bool value)
{
...
QDomElement
CSettings::openDomElement(QDomElement parent,
                          const QString& elementName,
                          const QString& attributeName)
{
  QDomElement wantedElement;

  debugSettings(("openDomElement1 %s %s %s",
                 qPrintable(parent.tagName()),
...
void
CSettings::loadHandSettings()
{
  if (m_domSong.isNull())
    return;
  m_domHand = openDomElement(
    m_domSong, "hand", partToHandString(m_song->getActiveHand()));
  // m_guiTopBar->setSpeed(m_domHand.attribute("speed", "100" ).toInt());
}

void
CSettings::saveHandSettings()
{
  // m_domHand.setAttribute("speed", m_guiTopBar->getSpeed());
}

void
CSettings::loadSongSettings()
{
...
CSettings::loadXmlFile()
{
  ...
  if (root.tagName() != "pianobooster") {
    m_domDocument.clear();
    QDomComment comment =
      m_domDocument.createComment("Piano Booster Configuration file");
...
void
CSettings::updateTutorPage()
{
  QFileInfo fileInfo(getCurrentSongLongFileName());
  const char* EXTN = ".html";

  QString fileBase =
    fileInfo.absolutePath() + "/InfoPages/" + fileInfo.completeBaseName() + "_";

  QString locale =
    value("General/lang", QLocale::system().bcp47Name()).toString();
...
void
CSettings::openSongFile(const QString& filename)
{
...
void
CSettings::setActiveHand(whichPart_t hand)
{
  saveHandSettings();
...
QStringList
CSettings::getSongList()
{
  debugSettings(("getSongList %s + %s",
                 qPrintable(getCurrentBookName()),
                 qPrintable(m_bookPath)));
...
void
CSettings::unzipBoosterMusicBooks()
{
  // Set default value
  ...
  if (value("PianoBooster/MusicRelease", 0).toInt() < MUSIC_RELEASE) {
    ...
    if (!QFile::exists(resourceDir + ZIPFILENAME)) {
      ...
      resourceDir = QApplication::applicationDirPath() + "/../share/games/" +
                    QSTR_APPNAME + "/music/";
#endif
#ifdef Q_OS_DARWIN
    ...
    ppLogInfo(
      qPrintable("applicationDirPath=" + QApplication::applicationDirPath()));
    ...
    destMusicDir.setPath(
      QDir::fromNativeSeparators(settings.value("Personal").toString()));
#else
    ...
    unzip.start("unzip",
                QStringList()
                  << "-o" << zipFile.filePath() << "-d" << destMusicDir.path());
    ppLogInfo(qPrintable("running unzip -o " + zipFile.filePath() + " -d " +
                         destMusicDir.path()));
    char buf[1024];
    while (true) {
...
#endif
    QString fileName(destMusicDir.absolutePath() + "/BoosterMusicBooks" +
                     QString::number(MUSIC_RELEASE) +
                     "/Booster Music/01-ClairDeLaLune.mid");
...
void
CSettings::setCurrentSongName(const QString& name)
{
  if (name.isEmpty())
    return;
  saveSongSettings();
  m_currentSongName = name;
  debugSettings(("setCurrentSongName %s -- %s",
                 qPrintable(name),
                 qPrintable(getCurrentSongLongFileName())));
  setValue("CurrentSong", getCurrentSongLongFileName());
...
void
CSettings::setCurrentBookName(const QString& name, bool clearSongName)
{
  if (name.isEmpty())
    return;
  ...
  debugSettings(("setCurrentBookName %s --- %s ",
                 qPrintable(name),
                 qPrintable(getCurrentSongLongFileName())));
  loadXmlFile();
}
...
void
CSettings::setChannelHands(int left, int right)
{
  CNote::setChannelHands(left, right);
...
void
CSettings::updateWarningMessages()
{
  if (!m_song->validMidiOutput())
    m_warningMessage =
      tr("ERROR NO SOUND: To fix this use menu Setup/Midi Setup ...");
  else if (m_currentSongName.isEmpty())
    m_warningMessage =
      tr("ERROR NO MIDI FILE: To fix this use menu File/Open ...");
  else
