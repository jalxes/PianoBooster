...
void
CSong::init2(CScore* scoreWin, CSettings* settings)
{
...
void
CSong::loadSong(const QString& filename)
{
  CNote::setChannelHands(-2, -2); // -2 for not set -1 for none
...
void
CSong::setActiveHand(whichPart_t hand)
{
...
void
CSong::setActiveChannel(int chan)
{
...
void
CSong::setPlayMode(playMode_t mode)
{
  regenerateChordQueue();
...
CSong::regenerateChordQueue()
{
  ...
  for (i = 0; i < length; i++) {
    ...
    if (m_findChord.findChord(event, getActiveChannel(), PB_PART_both) == true)
      chordEventInsert(
        m_findChord.getChord()); // give the Conductor the chord event
  }
  resetWantedChord();
}
...
CSong::task(int ticks)
{
  ...
  while (true) {
    ...
    while (true) {
      ...
      // ppLogTrace("Song event delta %d type 0x%x chan %d Note %d",
      // event.deltaTime(), event.type(), event.channel(), event.note());
    ...
    if (seekingBarNumber() && m_reachedMidiEof == false && playingMusic()) {
      realTimeEngine(0);
      m_scoreWin->drawScrollingSymbols(
        false); // don't display any thing just  remove from the queue
    } else
      break;
  }
...
bool
CSong::pcKeyPress(int key, bool down)
{
  int i;
  size_t j;
  CMidiEvent midi;
  ...
  if (key == 't') // the tab key on the PC fakes good notes
  {
    ...
    for (i = 0; i < m_fakeChord.length(); i++) {
      if (down)
        midi.noteOnEvent(0,
                         cfg_pcKeyChannel,
                         m_fakeChord.getNote(i).pitch() + getTranspose(),
                         cfg_pcKeyVolume);
      else
        midi.noteOffEvent(0,
                          cfg_pcKeyChannel,
  ...
  for (j = 0; j < arraySize(pcNoteLookup); j++) {
    if (key == pcNoteLookup[j].key) {
      if (down)
        midi.noteOnEvent(
          0, cfg_pcKeyChannel, pcNoteLookup[j].note, cfg_pcKeyVolume);
      else
        midi.noteOffEvent(
          0, cfg_pcKeyChannel, pcNoteLookup[j].note, cfg_pcKeyVolume);
