...
public:
  ...
  //! @param hand         the top (right hand) or the bottom the (left hand) see
  //! @ref whichPart_t.
  //! @param index        the save index number: 0 central line, 5 = top line,
  //! -5 the bottom line.
  ...
  float getPosY() { return verticalNoteSpacing() * m_staveIndex + m_offsetY; }
  ...
  float getPosYRelative()
  {
    return getPosY() - m_staveCenterY;
  } // get the Y position relative to the stave centre
...
private:
  // fixme TODO This could be improved as the calculations could a done in the
  // constructor
