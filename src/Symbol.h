...
@brief          A single musical symbols (eg note, rest, slur) that can appear
on the page.
...
#define BEAT_MARKER_OFFSET                                                     \
  20 // used to ensure that beat markers are drawn under the note by drawing
     // them early
...
public:
  ...
  CSymbol(musicalSymbol_t type,
          CStavePos stavePos,
          CColor color = Cfg::noteColor())
  {
    init();
    m_symbolType = type;
    m_stavePos = stavePos;
  ...
  int getAccidental() { return getStavePos().getAccidental(); }

  void setAccidentalModifer(accidentalModifer_t value)
  {
    m_accidentalModifer = value;
  }
...
private:
  ...
  byte m_midiNote;
  accidentalModifer_t
    m_accidentalModifer; // Used to suppress the second sharp in the same bar
  int m_midiDuration;
  whichPart_t m_hand;
