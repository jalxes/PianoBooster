...
void
CTrackList::init(CSong* songObj, CSettings* settings)
{
  m_song = songObj;
  m_settings = settings;
}
...
void
CTrackList::currentRowChanged(int currentRow)
{
  if (!m_song)
    return;
...
void
CTrackList::examineMidiEvent(CMidiEvent event)
{
  int chan;
  chan = event.channel();
...
{
  if ((m_midiFirstPatchChannels[CONVENTION_LEFT_HAND_CHANNEL] ==
         GM_PIANO_PATCH &&
       m_midiActiveChannels[CONVENTION_LEFT_HAND_CHANNEL] == true &&
       m_midiFirstPatchChannels[CONVENTION_RIGHT_HAND_CHANNEL] <=
         GM_PIANO_PATCH))
    return true;

  if (m_midiFirstPatchChannels[CONVENTION_RIGHT_HAND_CHANNEL] ==
        GM_PIANO_PATCH &&
...
int
CTrackList::guessKeySignature(int chanA, int chanB)
{
  int chan;
  int i;
  int keySignature = 0;
...
int
CTrackList::findFreeChannel(int startChannel)
{
  int chan;
...
CTrackList::refresh()
{
  ...
  if (pianoPartConvetionTest()) {
    if (CNote::bothHandsChan() == -2) // -2 for not set -1 for not used
      CNote::setChannelHands(CONVENTION_LEFT_HAND_CHANNEL,
                             CONVENTION_RIGHT_HAND_CHANNEL);
  ...
  if (CStavePos::getKeySignature() == NOT_USED)
    CStavePos::setKeySignature(
      guessKeySignature(CNote::rightHandChan(), CNote::leftHandChan()), 0);
...
QStringList
CTrackList::getAllChannelProgramNames(bool raw)
{
  QStringList items;
  int chan;
  QString text;
...
int
CTrackList::getActiveHandIndex(whichPart_t whichPart)
{
  int index = 0;
  for (int i = 0; i < m_trackQtList.size(); ++i)
...
void
CTrackList::setActiveHandsIndex(int leftIndex, int rightIndex)
{
  int leftChannel = -1;
  int rightChannel = -1;
...
int
CTrackList::getHandTrackIndex(whichPart_t whichPart)
{
  int index = 0;
...
void
CTrackList::changeListWidgetItemView(unsigned int index,
...
QString
CTrackList::getChannelProgramName(int chan)
{
  if (chan < 0 ||
      chan >= static_cast<int>(arraySize(m_midiFirstPatchChannels))) {
    assert(true);
    return QString();
  }
...
QString
CTrackList::getProgramName(int program)
{
