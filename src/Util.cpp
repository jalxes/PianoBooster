...
void
ppLog(logLevel_t level, const char* msg, ...)
{
  va_list ap;
...
void
benchMarkReset(benchData_t* pBench)
{
  pBench->deltaTotal = 0;
  pBench->deltaCount = 0;
  pBench->maxDelta = 0;
...
int
benchMarkUpdate(benchData_t* pBench, int previousTime)
{
...
void
benchMark(unsigned int id, QString message)
{
...
void
printResult(int i, benchData_t* pBench)
{
  ...
  fprintf(logInfoFile,
          "ct %4d, min %2d, avg %4.3f, max %2d frame %4.3f %s\n",
          pBench->deltaCount,
          pBench->minDelta,
          static_cast<double>(pBench->deltaTotal) / pBench->deltaCount,
          pBench->maxDelta,
          (static_cast<double>(pBench->frameRateCurrent -
                               pBench->frameRatePrevious)) /
            pBench->deltaCount,
